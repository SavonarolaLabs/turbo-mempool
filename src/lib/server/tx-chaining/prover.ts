...
import type { ProverDeviceState, SigningState, StateAddress } from "../../types/internal";
...
export class Prover {
    ...
    public generateCommitments(tx) {
        const wallet = this.buildWallet(this._from, this._deriver);
        const commitments = wallet.generate_commitments_for_reduced_transaction(tx);
        return this._extractCommitments(commitments, tx.unsigned_tx().inputs().len());
    }
    ...
    private _extractCommitments = (commitment: TransactionHintsBag, inputLength: number) => {
    ...
    public signMulti(tx: Transaction, context: ErgoStateContext, hints: TransactionHintsBag) {
        const wallet = this.buildWallet(this._from, this._deriver);
        const sigmaRust = wasmModule.SigmaRust;
        const dataInputBoxes = [];
        return wallet.sign_transaction_multi(context, tx, tx.inputs, dataInputBoxes, hints);
