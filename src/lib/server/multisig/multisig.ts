...
export async function signTx(mnemonic: string, address: string, tx: EIP12UnsignedTransaction): Promise<wasm.Transaction> {
    const prover = await getProver(mnemonic);

    // filtet  == address 
    const boxesToSign = tx.inputs.filter(i => i.ergoTree == ErgoAddress.fromBase58(address).ergoTree)
    const boxes_to_spend = ErgoBoxes.empty();
    boxesToSign.forEach(box =>{
        boxes_to_spend.add(ErgoBox.from_json(JSON.stringify(box)))
    })

    const signedTx = prover.sign_transaction(fakeContext(), wasm.UnsignedTransaction.from_json(JSON.stringify(tx)), boxes_to_spend, ErgoBoxes.empty())
    return signedTx;
};
