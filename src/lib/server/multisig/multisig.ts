...
export async function signMultisig(
	...
	return signedTx; // this all input Partial+Partial...   1 input - Partial+Partial , 2 input - 1Sign
}
...
export function arrayToProposition(input: Array<string>) {
...
export async function signMultisigPart1(unsignedTx: EIP12UnsignedTransaction) {
	await wasmModule.loadAsync();
	const shadow = { mnemonic: SHADOW_MNEMONIC, address: SHADOWPOOL_ADDRESS };
	const proverBob = await getProver(shadow.mnemonic);
	const hBob = ErgoAddress.fromBase58(shadow.address).ergoTree.slice(6);
	...
	return { transaction: reducedTx, transactionHintsBag: hintsForAliceSign }; // this all input Partial+Partial...   1 input - Partial+Partial , 2 input - 1Sign
} // tx + commit

export async function signMultisigPart2(
	...
	// ?
	for (var i = 0; i < reducedTx.unsigned_tx().inputs().len(); i++) {
		hintsAll.add_hints_for_input(
			i,
			initialCommitsAlice.all_hints_for_input(i)
		);
	...
	return hintsAll; // this all input Partial+Partial...   1 input - Partial+Partial , 2 input - 1Sign
} // sign1 + commit2

export async function signMultisigPart3(
	unsignedTx: EIP12UnsignedTransaction,
	userMnemonic: string,
	userAddress: string
) {
