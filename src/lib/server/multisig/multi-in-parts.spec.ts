...
describe('buy', () => {
	...
	it('signPartA signPartB signPartC', async () => {
		const token = {
			name: 'TestToken Test2',
			tokenId:
				'b73a806dee528632b8d76f07813a1f1b66b8e11bc32b3ad09f8051265f3664ab',
		...
		const { privateCommitsBob, publicCommitsBob } = await a(buyOrderUTx);
		...
		const buyOrderTx = (
			await c(buyOrderUTx, privateCommitsBob, extractedHints)
		).to_js_eip12();
