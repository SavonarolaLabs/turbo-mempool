...
describe.only('Multiple Keys in one register', () => {
	beforeAll(async()=>{
		const output = new OutputBuilder(
			...
			R5: SColl(SSigmaProp, [
				SGroupElement(first(ErgoAddress.fromBase58(ALICE_ADDRESS).getPublicKeys())),
		...
		const signed = await signTxAllInputs(BOB_MNEMONIC, BOB_ADDRESS, unsigned);
		...
		const signed2 = await signTxAllInputs(BOB_MNEMONIC, BOB_ADDRESS, unsigned2);
	...
	it('alice can sign if R4 == 1', async () => {
		...
		const signed = await signTxAllInputs(ALICE_MNEMONIC, ALICE_ADDRESS, unsigned);
	...
	it('bob CANT sign if R4 == 1', async () => {
		...
		const unsigned = new TransactionBuilder(currentHeight)
			...
			expect(() => signTxAllInputs(BOB_MNEMONIC, BOB_ADDRESS, unsigned)).rejects.toThrowError();
	});
	...
	it('alice CANT sign if R4 == 2', async () => {
		...
		const unsigned = new TransactionBuilder(currentHeight)
			...
			expect(signTxAllInputs(ALICE_MNEMONIC, ALICE_ADDRESS, unsigned)).rejects.toThrowError();
	});
