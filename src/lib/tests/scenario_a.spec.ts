...
import { ergoTree } from '$lib/utils/helper';
import { depositMultiple } from '$lib/wallet/deposit';
...
describe('screnario A', () => {
	...
	it('box tied to userPk', () => {
		const boxUsers: string[] = db.boxRows.map(
			(row) => row.box.additionalRegisters.R4 ?? ''
		);
		const user1Deposit = boxUsers.find((r4) =>
			r4.includes(ergoTree(userA.address).slice(4))
		);
